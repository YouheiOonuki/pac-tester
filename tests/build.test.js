// ビルドした pac-tester.html（ダウンロード版）のテスト: node --test tests/*.test.js
...
const html = fs.readFileSync(path.join(ROOT, 'pac-tester.html'), 'utf8');
const read = (p) => fs.readFileSync(path.join(ROOT, p), 'utf8');
const OUR_CODE = ['src/core.js', 'src/app.js', 'src/pac-runtime.js', 'src/worker.js', 'src/sandbox.html'];
...
test('PAC は sandbox の iframe（allow-same-origin なし）の中の Worker で評価する', () => {
  ...
  assert.match(html, /window\.PT_SANDBOX_HTML = "/);
});
...
test('ライブラリとライセンス全文が入っている', () => {
  ...
  for (const s of ['acorn@', 'MIT License', 'Permission is hereby granted', 'Copyright (C) 2012-2022 by various contributors']) {
    ...
    assert.ok(html.includes(s), 'pac-tester.html に ' + s);
  }
});
...
test('script 要素が途中で閉じていない（埋め込んだコードに </script が残っていない）', () => {
  ...
  assert.equal(opens, 5);   // acorn・core.js・隔離用の枠・app.js・ライセンス
});

test('index.html・guide.html はダウンロード版へリンクし、ダウンロード版はサイトマップに載せない', () => {
  const index = read('index.html');
  assert.match(index, /<a [^>]*href="\.\/pac-tester\.html"[^>]*\sdownload/);
  assert.doesNotMatch(read('sitemap.xml'), /pac-tester\.html/);
});
